									...
									<%-- <%@ page import=" java.sql.Connection"%>
										...
										<%
											try {
													...
													String sql = "select distinct (select count(type) from sewwandi.leave  where  EmpID = '"+empID+"' and date LIKE '2020-09-%'and type ='Full Day') as countFull, (select count(type) from sewwandi.leave  where  EmpID = '"+empID+"' and date LIKE '2020-09-%'and type ='Half Day') as countHalf from sewwandi.leave ";
													resultSet = statement.executeQuery(sql);
										%>
										<%=resultSet.getString("countFull")%>
										<%=resultSet.getString("countHalf")%>
										<%
											connection.close();
 ...
 --%>
								<form onsubmit="return validate()" action="insetLeave"
									...
									<c:forEach var="emp" items="${empDetails}">
										<c:set var="id" value="${emp.employeeId}" />	
