...
public class AddMenuOrderServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(isTrue == true) {
			...
			RequestDispatcher dis = request.getRequestDispatcher("http://localhost:8080/Hotel_SEWWANDI/MenuOrderDetails.jsp");
			dis.forward(request, response);
			
		
			 }else { RequestDispatcher dis2 =
			  request.getRequestDispatcher("http://localhost:8080/Hotel_SEWWANDI/HallBooking.jsp"); dis2.forward(request,
			  response); }
