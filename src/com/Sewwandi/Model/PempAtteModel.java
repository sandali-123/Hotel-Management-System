...
String e1 = null;	
	...
	public String insertEmp(AttenBean at) {
		...
		try {
			...
			stmt.executeUpdate("INSERT INTO sewwandi.per_empattendan(date,employeeId,firstName, lastName,section, designation, status,basicSalary, clockin,clockout,work_hrs,no_OT,OT)VALUE ('"+at.getDate()+"','"+at.getEmployeeId()+"','"+at.getFirstName()+"','"+at.getLastName()+"',  '"+at.getSection()+"', '"+at.getDesignation()+"', '"+at.getStatus()+"','"+at.getBasicsalary()+"','"+at.getClockin()+"','"+at.getClockout()+"',timediff( '"+at.getClockout()+"','"+at.getClockin()+"'),case when TIMEDIFF( '"+at.getClockout()+"','"+at.getClockin()+"') <= '07:00' then TIMEDIFF( '"+at.getClockout()+"','"+at.getClockin()+"') else '07:00:00' end,case when TIMEDIFF( '"+at.getClockout()+"','"+at.getClockin()+"') > '07:00' then TIMEDIFF(TIMEDIFF(  '"+at.getClockout()+"','"+at.getClockin()+"') ,'07:00')  else '00:00:00' end  )");
			e1 = "call inserted";
